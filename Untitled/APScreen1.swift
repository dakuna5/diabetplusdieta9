...
public class APScreen1 : ANViewController, UITableViewDataSource, UITableViewDelegate, ANDataListDelegate  {
    ...
    @IBAction func swipeRight(sender: Any?) {
        self.performSegue(withIdentifier: "screen1 -> screen3 - Push", sender: self)
    }
    ...
    public func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if (tableView == self.userstablesmall) {
            let cell = tableView.dequeueReusableCell(withIdentifier: "usercell") as! APUsercell;
